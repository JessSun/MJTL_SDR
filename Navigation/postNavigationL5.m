function [navSolutions, eph] = postNavigationL5(trackResults, settings)
...
for channelNr = activeChnList
    ...
    Integrated_Doppler(channelNr,:)=(settings.IF-trackResults(channelNr).carrFreq);
end
...
for currMeasNr = 1:measNrSum
    ...
    % These two lines help the skyPlot function. The satellites excluded do
    % to elevation mask will not "jump" to possition (0,0) in the sky plot.
    ...
    [navSolutions.rawP(:, currMeasNr),transmitTime,localTime]= calculatePseudoranges(trackResults,subFrameStart,TOW, ...
        currMeasSample,localTime,activeChnList, settings);

    navSolutions.rawdopper(:,currMeasNr)= calculatePhaseranges(settings,Integrated_Doppler,subFrameStart,currMeasNr,activeChnList)';

    navSolutions.rawL(:,currMeasNr) = navSolutions.rawdopper(:,currMeasNr);
    if currMeasNr>1
        navSolutions.rawL(:,currMeasNr) = navSolutions.rawL(:,currMeasNr-1)+navSolutions.rawdopper(:,currMeasNr);
    end
    ...
    [satPositions, satClkCorr] = satpos_L5(transmitTime(activeChnList), [trackResults(activeChnList).PRN], eph);
    ...
    if size(activeChnList, 2) > 3
        ...
        navSolutions.correctedP(activeChnList, currMeasNr) = ...
            navSolutions.rawP(activeChnList, currMeasNr) + satClkCorr' * settings.c - xyzdt(4);

        %% Coordinate conversion ==================================================

        %=== Convert to geodetic coordinates ==============================
        [navSolutions.latitude(currMeasNr), navSolutions.longitude(currMeasNr), navSolutions.height(currMeasNr)] =  ...
            cart2geo(navSolutions.X(currMeasNr), navSolutions.Y(currMeasNr), navSolutions.Z(currMeasNr),5);

        %=== Convert to UTM coordinate system =============================
        navSolutions.utmZone = findUtmZone(navSolutions.latitude(currMeasNr), navSolutions.longitude(currMeasNr));

        % Position in ENU
        [navSolutions.E(currMeasNr), navSolutions.N(currMeasNr),navSolutions.U(currMeasNr)] = ...
            cart2utm(xyzdt(1), xyzdt(2), xyzdt(3),navSolutions.utmZone);

    else
        %--- There are not enough satellites to find 3D position ----------
        disp(['   Measurement No. ', num2str(currMeasNr), ': Not enough information for position solution.']);

        %--- Set the missing solutions to NaN. These results will be excluded automatically in all plots. 
        % For DOP it is easier to use zeros. NaN values might need to be excluded from results in some of
        % further processing to obtain correct results.
