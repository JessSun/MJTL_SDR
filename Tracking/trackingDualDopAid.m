function [trackResultsL1,trackResultsL5]= trackingDualDopAid(channel, settings)
% Doppler-aided joint tracking function for GPS L1/L5 signals.
%
% [trackResultsL1,trackResultsL5]= trackingDualDopAid(channel, settings)
%
%   Inputs:
...
%% Initialization of L1 tracking variables ================================
...
%% Initialization of L5 tracking variables ================================
...
[tau1codeL1, tau2codeL1] = calcLoopCoef(settings.dllNoiseBandwidth1, settings.dllDampingRatio1,1.0);
[tau1codeL5, tau2codeL5] = calcLoopCoef(settings.dllNoiseBandwidth5, settings.dllDampingRatio5, 1.0);
% PLL variables
[tau1carrL1, tau2carrL1] = calcLoopCoef(settings.pllNoiseBandwidth1, settings.pllDampingRatio1, 0.25);
[pf3,pf2,pf1] = calcLoopCoefCarr(settings,settings.pllNoiseBandwidth5);
...
beta = settings.codeFreqBasis1/settings.codeFreqBasis5;
...
for channelNr = 1:length(channel)
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        fseek(fidL1, settings.fileType*(settings.skipNumberOfBytes + channel(channelNr).codePhaseL1-1),'bof');
        fseek(fidL5, settings.fileType*(settings.skipNumberOfBytes + channel(channelNr).codePhaseL5-1),'bof');
        ...
        codeFreqL1      = channel(channelNr).codeFreqL1;
        % Define residual code phase (in chips)
        remCodePhaseL1  = 0.0;
        % Define carrier frequency which is used over whole tracking period
        carrFreqL1      = channel(channelNr).acquiredFreqL1;
        carrFreqBasisL1 = channel(channelNr).acquiredFreqL1;      
        ...
        % define initial code frequency basis of NCO
        codeFreqL5      = channel(channelNr).codeFreqL5;
        % Define residual code phase (in chips)
        remCodePhaseL5  = 0.0;
        % Define carrier frequency which is used over whole tracking period
        carrFreqL5      = channel(channelNr).acquiredFreqL5;
        carrFreqBasisL5 = channel(channelNr).acquiredFreqL5;      
        % Define residual carrier phase
        remCarrPhaseL5  = 0.0;
        % code tracking loop parameters
        oldCodeNcoL5   = 0.0;
        ...
        for loopCnt =  1:codePeriods
            ...
            trackResultsL1(channelNr).absoluteSample(loopCnt) =(ftell(fidL1))/settings.fileType;
            trackResultsL5(channelNr).absoluteSample(loopCnt) =(ftell(fidL5))/settings.fileType;
            ...
            [rawSignalL1, samplesReadL1] = fread(fidL1, settings.fileType*blksizeL1, settings.dataType);
            ...
            if (samplesReadL1 ~= settings.fileType*blksizeL1)
            ...
            carrNcoL1 = oldCarrNcoL1 + (tau2carrL1/tau1carrL1) * (carrErrorL1 - oldCarrErrorL1) + carrErrorL1 * (PDIcarr/tau1carrL1);
            oldCarrNcoL1   = carrNcoL1;
            oldCarrErrorL1 = carrErrorL1;         
            %% Find DLL error and update code NCO -------------------------------------
            codeErrorL1 = (1-earlyLateSpcL1)*(sqrt(I_E * I_E + Q_E * Q_E) - sqrt(I_L * I_L + Q_L * Q_L)) / ...
                (sqrt(I_E * I_E + Q_E * Q_E) + sqrt(I_L * I_L + Q_L * Q_L));
            codeNcoL1 = oldCodeNcoL1 + (tau2codeL1/tau1codeL1) * (codeErrorL1 - oldCodeErrorL1) + codeErrorL1 * (PDIcode/tau1codeL1);
            oldCodeNcoL1   = codeNcoL1;
            oldCodeErrorL1 = codeErrorL1;
            ...
            % Read in the appropriate number of samples to process this interation
            [rawSignalL5, samplesReadL5] = fread(fidL5,settings.fileType*blksizeL5, settings.dataType);
            ...
            % If did not read in enough samples, then could be out of data - better exit
            if (samplesReadL5 ~= settings.fileType*blksizeL5)
                disp('Not able to read the specified number of samples  for tracking, exiting!')
                delete(hwb);
                return
            end
            ...
            d2CarrErrorL5 = d2CarrErrorL5 + carrErrorL5 * pf3;
            dCarrErrorL5  = d2CarrErrorL5 + carrErrorL5 * pf2 + dCarrErrorL5;
            carrNcoL5     = dCarrErrorL5 + carrErrorL5 * pf1;
            ...
            codeNcoL5 = oldCodeNcoL5 + (tau2codeL5/tau1codeL5) * (codeErrorL5 - oldCodeErrorL5) + codeErrorL5 * (PDIcode/tau1codeL5);
            oldCodeNcoL5   = codeNcoL5;
            oldCodeErrorL5 = codeErrorL5;
            trackResultsL5(channelNr).codeFreq(loopCnt) = codeFreqL5;
            % Modify code freq based on NCO command
            codeFreqL5 = channel(channelNr).codeFreqL5 + codeNcoL5;

            trackResultsL1(channelNr).codeFreq(loopCnt) = codeFreqL1;
            trackResultsL1(channelNr).carrFreq(loopCnt) = carrFreqL1;

            %% Doppler‑aided multi-frequency tracking Loop
            % Calculate carrier frequency of L1 signal aided by carrier doppler of L5
            carrFreqL1 = carrFreqBasisL1 + carrNcoL1 + alpha*carrNcoL5;
            % Calculate code frequency of L1 signal aided by code doppler of L5
            codeFreqL1 = channel(channelNr).codeFreqL1 + codeNcoL1 + beta*codeNcoL5;

            %% Record various measures to show in postprocessing ----------------------
            trackResultsL1(channelNr).PRN                     = channel(channelNr).PRN;
            trackResultsL1(channelNr).dllDiscr(loopCnt)       = codeErrorL1;
            trackResultsL1(channelNr).dllDiscrFilt(loopCnt)   = codeNcoL1 + beta*codeNcoL5;
            trackResultsL1(channelNr).pllDiscr(loopCnt)       = carrErrorL1;
            trackResultsL1(channelNr).pllDiscrFilt(loopCnt)   = carrNcoL1 + alpha*carrNcoL5;
            ...
            if (rem(loopCnt,settings.CNo.VSMinterval)==0)
                % L1
                vsmCntL1=vsmCntL1+1;
                CNoValue=CNoVSM(trackResultsL1(channelNr).I_P(loopCnt-settings.CNo.VSMinterval+1:loopCnt),...
                    trackResultsL1(channelNr).Q_P(loopCnt-settings.CNo.VSMinterval+1:loopCnt),settings.CNo.accTime);
                trackResultsL1(channelNr).CNo.VSMValue(vsmCntL1)=CNoValue;
                trackResultsL1(channelNr).CNo.VSMIndex(vsmCntL1)=loopCnt;
                CNoL1=int2str(CNoValue);
                % L5
                vsmCntL5=vsmCntL5+1;
                CNoValue=CNoVSM(trackResultsL5(channelNr).I_P(loopCnt-settings.CNo.VSMinterval+1:loopCnt),...
                    trackResultsL5(channelNr).Q_P(loopCnt-settings.CNo.VSMinterval+1:loopCnt),settings.CNo.accTime);
                trackResultsL5(channelNr).CNo.VSMValue(vsmCntL5)=CNoValue;
                trackResultsL5(channelNr).CNo.VSMIndex(vsmCntL5)=loopCnt;
                CNoL5=int2str(CNoValue);
            end
