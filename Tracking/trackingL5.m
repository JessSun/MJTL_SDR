function [trackResults, channel]= trackingL5(channel, settings)
% Performs code and carrier tracking for all channels.
%
%[trackResults] = tracking(fid, channel, settings)
%
%   Inputs:
...
%--- PLL variables --------------------------------------------------------
% Calculate filter coefficient values
[pf3,pf2,pf1] = calcLoopCoefCarr(settings,settings.pllNoiseBandwidth);
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        for loopCnt =  1:codePeriods
           ...
           %% Generate the carrier frequency to mix the signal to baseband -----------
            ...
            % chaannel signals
            if (settings.pilotTRKflag == 1)
                codeErrorQ = (1-earlyLateSpc)*(sqrt(I_EQ^2 + Q_EQ^2) - sqrt(I_LQ^2 + Q_LQ^2)) / ...
